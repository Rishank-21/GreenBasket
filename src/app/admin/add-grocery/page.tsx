...
const handleSubmit = async (e:FormEvent)=> {
  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-linear-to-b from-green-50 to-white py-16 px-4 relative'>
      <Link href={'/'} className='absolute top-6 left-6 flex items-center justify-center gap-2 text-green-700 font-semibold bg-white px-4 py-2 rounded-full shadow-md  hover:bg-green-100 hover:shadow-lg transition-all '>
    ...
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
     ...
     <div>
        <label htmlFor="price" className='block text-gray-700 font-medium mb-1'>Grocery Price<span className='text-red-500'>*</span></label>
        <input type="text" id='price'  placeholder='Eg. 520' className='w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-green-400 transition-all' onChange={(e) => setPrice(e.target.value)} value={price}/>
      </div>
      ...
      <div className='flex flex-col sm:flex-row items-center gap-5'>
        ...
        {previewImage && <Image src={previewImage} width={100} height={100} alt='grocery-image' className='rounded-xl shadow-md border border-gray-200 object-cover'/>}
      </div>
          <motion.button
          whileHover={{scale:1.02}}
          whileTap={{scale:0.9}}
          disabled={loading}
          className='mt-4 w-full bg-linear-to-b from-green-500 to-green-700 text-white font-semibold py-3 rounded-xl shadow-lg hover:shadow-xl disabled:opacity-60 transition-all flex items-center justify-center gap-2'
          >
