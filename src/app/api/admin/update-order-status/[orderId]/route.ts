...
try {
    ...
    console.log("update-order-status called with orderId:", orderId)
    const {status} = await req.json()
    // validate ObjectId early to avoid silent null results
    if(!mongoose.Types.ObjectId.isValid(orderId)){
        return NextResponse.json({message:"Invalid order id"}, {status:400})
    }
    let order = await Order.findByIdAndUpdate(orderId, {status}, {new:true}).populate("user")
    if(!order){
    ...
    if(status==="out of delivery" && !order.assignment){
       ...
       if(candidates.length === 0){
        ...
        return NextResponse.json({message:"No available delivery boys"}, {status:200})
       }
