...
const UserOrderCard = ({order}:{order:IOrder}) => {
  ...
  return (
    <motion.div initial={{opacity:0, y:15}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className='bg-white rounded-2xl border border-gray-100 shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden'>
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 bg-linear-to-r from-green-50 to-white'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
            ...
            <span className={`${getStatusColor(status)} px-3 py-1 text-xs font-semibold border rounded-full`}>{status}</span>
        </div>
      </div>
    ...
    {status !== "delivered" && (
        ...
        {order.paymentMethod==="cod" ? <div className='flex items-center gap-2 text-gray-700 text-sm
...

                 {
                  ...
                  <div className="mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4
                   flex items-center justify-between">
                    ...
                    <a href={`tel:+91${order.assignedDeliveryBoy?.mobile}`} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                      Call
                    </a>
                  </div>

                  <button className='w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold px-4 py-2 rounded-xl shadow hover:bg-green-700 transition cursor-pointer' onClick={()=>router.push(`/user/track-order/${order._id?.toString()}`)}><Truck className='text-white' size={16}/> Track Your Order</button>
            ...
            <div className='border-t border-gray-200 pt-3'>
                <button onClick={()=>setExpanded(prev=>!prev)} className='w-full flex justify-between items-center text-sm font-medium text-gray-700 hover:text-green-700 transition'>
                    
                    <span className='flex items-center gap-2'>
                        <Package className='text-green-600' size={16}/>
                        {expanded ? "Hide Order Item" : `View ${order.items.length} Items`}
                    </span>
                ...
                >
                    <div className='mt-3 space-y-3'>
                        {order.items.map((item,idx)=>(
                            <div key={idx} className='flex justify-between items-center bg-gray-50 rounded-xl px-3 py-2 hover:bg-gray-100 transition'>
