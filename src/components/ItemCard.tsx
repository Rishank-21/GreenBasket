...
const ItemCard = ({ item }: { item: IGrocery }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        >
          <button className="w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all cursor-pointer" onClick={() => dispatch(decreaseQuantity(item._id))}><Minus size={16} className="text-green-700"/></button>
          <span className="text-sm font-semibold text-gray-800">{cartItem.quantity}</span>
          <button className="w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all cursor-pointer" onClick={() => dispatch(increaseQuantity(item._id))}><Plus size={16} className="text-green-700"/></button>
          </motion.div>}
